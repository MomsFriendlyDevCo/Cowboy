...
export default class CowboyResponse {
	...
	*
	* @param {Function} cb Async function to queue. Will be called as `(req:CowboyRequest, res:CowboyResponse)` - use `res.body` to access the body, res can be mutated before being served
	* @returns {CowboyResponse} This chainable instance
	*/
	beforeServe(cb) {
	...
	/**
	* Queue up a function to run if an error occured during any middleware block
	*
	* @param {Function} cb Async function to queue. Will be called as `(req:CowboyRequest, res:CowboyResponse, err:Error)`
	* @returns {CowboyResponse} This chainable instance
	*/
	afterError(cb) {
		this.afterErrorCallbacks.push(cb);
		return this;
	}
	...
	*/
	async toCloudflareResponse(req) {
		...
		if (!this.hasReturnedCF) await Array.fromAsync(this.beforeServeCallbacks, cb =>
			cb.call(this, req, this)
		);
		...
		// Mark that we have computed the Cloudflare response at least once - invalidates beforeServe callbacks on next use
		this.hasReturnedCF = true;
