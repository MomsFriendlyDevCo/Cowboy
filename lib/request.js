	...
	async parseBody(forceType) {
		...
		switch (type) {
			...
			case 'application/json':
				if (
					!this.headers['content-length'] // Content type is claimed as JSON but there is no length - assume blank
					|| this.headers['content-length'] == 0 // Sending JSON but its blank
				) {
					this.body = {};
					...
					try {
						...
						throw new Error('Invalid JSON body', {cause: e});
					}
				}
				break;
			case 'formData':
			...
			case 'application/x-www-form-urlencoded': // Decode as multi-part
				try {
					...
					throw new Error('Invalid multi-part encoded body', {cause: e});
				}
				break;
			case 'text':
			case 'text/plain': // Decode as plain text
				try {
					...
					throw new Error('Invalid text body', {cause: e});
				}
				break;
			default:
